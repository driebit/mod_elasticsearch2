-module(mod_elasticsearch2_tests).
...
build_query_test() ->
    {Query, _QArgs} = elasticsearch2_search:build_query([], {1,100}, context()),
...
build_query_sort_random_test() ->
    Seed = 10,
    {Query, _QArgs} = elasticsearch2_search:build_query([{sort,<<"random">>}, {seed, Seed}], {1,100}, context()),
    ?assertEqual(Query, #{<<"aggregations">> => #{},
                          <<"query">> =>
                              #{<<"function_score">> =>
                                    #{<<"boost_mode">> => <<"sum">>,
...
map_test() ->
    ...
    Mapped = elasticsearch2_mapping:map_rsc(Id, context()),
...
start_module(Context) ->
    ok = z_module_manager:activate_await(mod_elasticsearch2, Context).
